...
(function() {
  ...
  rw.instance_dsl = {
    ...
    rw: function() {
      var action, d, key, key_name, raw_value, v, value, _ref;
      if ((_ref = this._rw_data_) == null) {
        this._rw_data_ = {};
      }
      d = this._rw_data_;
      action = arguments.length === 1 ? 'read' : arguments.length === 2 ? 'write' : void 0;
      ...
      switch (action) {
        case 'write':
          return d[key.name()] = key.is_bool() ? !!raw_value : raw_value;
        case 'read':
          value = d[key.name()];
          if (key.is_bool()) {
            return !!value;
